...
CREATE OR REPLACE FUNCTION "public"."enqueue_welcome_program_email"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_amount NUMERIC := COALESCE(
    (NEW.meta->>'transaction_amount')::NUMERIC,
    (NEW.meta->>'amount')::NUMERIC,
    0
  );
BEGIN
  -- Solo si la compra está aprobada y es de upsell
  IF NEW.status = 'approved'
     AND v_amount >= 75
     AND NOT EXISTS (
       SELECT 1 FROM public.outbox_emails
       WHERE payload->>'session_id' = NEW.session_id
  ...
  THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'customer_email', COALESCE(NEW.email, ''),
        'amount', v_amount
      ),
      'queued'
    );
...
ALTER FUNCTION "public"."enqueue_welcome_program_email"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."run_email_worker_cleanup"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
begin
  -- 1️⃣ Eliminar correos de prueba o nulos
  delete from outbox_emails
  where to_email ilike '%@testuser.com%' or to_email is null;
  ...
  set attempts = 0,
      last_error = null,
      updated_at = now()
  where status = 'queued' and attempts > 50;

  -- 3️⃣ Eliminar correos antiguos ya procesados
  delete from outbox_emails
  where status in ('sent', 'delivered')
    and updated_at < now() - interval '60 days';

  -- 4️⃣ Eliminar correos fallidos de más de 30 días
  delete from outbox_emails
  where status in ('failed', 'bounced')
    and updated_at < now() - interval '30 days';

  -- 5️⃣ (opcional) Log para registro
  insert into maintenance_logs (message, created_at)
  values ('Limpieza ejecutada manualmente desde SQL Editor', now())
  on conflict do nothing;
end;
$$;


ALTER FUNCTION "public"."run_email_worker_cleanup"() OWNER TO "postgres";
...
begin
  ...
  set email_final = v_email, updated_at = now()
  ...
  update public.purchases
  set email = v_email
  where session_id = p_session_id and (email is null or email = '');

  -- 4. INSERTAR CORREO UPSELL (Programa Completo)
  -- Si el ID contiene "programa", enviamos el template 'welcome-program'
  if v_product_id ilike '%programa%' or v_product_id ilike '%premium%' then
    insert into public.outbox_emails (to_email, template, payload, status)
    select v_email,
           'welcome-program',
           jsonb_build_object('session_id', p_session_id, 'customer_email', v_email),
           'queued'
    where not exists (
      select 1 from public.outbox_emails
      where to_email = v_email and template = 'welcome-program' and payload->>'session_id' = p_session_id
    );
    return;
  end if;

  -- 5. INSERTAR CORREO KIT (Default para todo lo demás)
  insert into public.outbox_emails (to_email, template, payload, status)
  select v_email,
         'welcome-kit',
         jsonb_build_object('session_id', p_session_id, 'customer_email', v_email),
  ...
  where not exists (
    select 1 from public.outbox_emails
    where to_email = v_email and template = 'welcome-kit' and payload->>'session_id' = p_session_id
  );
end;
$$;
...
ALTER TABLE "public"."security_audit_log" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
...
GRANT ALL ON FUNCTION "public"."enqueue_welcome_program_email"() TO "anon";
GRANT ALL ON FUNCTION "public"."enqueue_welcome_program_email"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."enqueue_welcome_program_email"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."run_email_worker_cleanup"() TO "anon";
GRANT ALL ON FUNCTION "public"."run_email_worker_cleanup"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."run_email_worker_cleanup"() TO "service_role";
