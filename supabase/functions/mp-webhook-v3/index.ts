...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const safeEmail = emailFromSession && !emailFromSession.endsWith('@testuser.com') ? emailFromSession : !isMPTestUser ? emailFromMP : null;
    ...
    const { error: upsertErr } = await supabase.from('purchases').upsert(row, {
    ...
    if (safeEmail) {
      await supabase.from('purchases').update({
        email: safeEmail
      }).eq('provider', 'mercadopago').eq('provider_payment_id', String(paymentId)).is('email', null);
    }
