...
begin
  ...
  delete from public.outbox_emails
  where to_email = v_email
    and status in ('queued','sent')
    and payload->>'session_id' <> p_session_id;

  if v_product_id in ('kit-reinicio-01','kit','rm-kit','7usd') then
    insert into public.outbox_emails (to_email, template, payload, status)
    select v_email,
           'welcome-kit',
           jsonb_build_object(
  ...
  if v_product_id in ('premium-rm-75','premium','premium-75','programa75') then
    insert into public.outbox_emails (to_email, template, payload, status)
    select v_email,
           'welcome-program',
           jsonb_build_object(
