// src/pages/Planeador.jsx (v3.0 - Final Release)
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
        <div className="bg-slate-900 border border-slate-700 w-full max-w-2xl max-h-[80vh] rounded-2xl flex flex-col shadow-2xl">
          <div className="p-6 border-b border-slate-800 flex justify-between items-center">
            ...
            <button onClick={() => setSelectedDaySlot(null)} className="text-slate-400 hover:text-white text-xl">&times;</button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button 
                onClick={() => { updateSlot(selectedDaySlot.day, selectedDaySlot.slotId, null); setSelectedDaySlot(null); }}
            ...
            {compatibleRecipes.map(recipe => (
              <button 
                key={recipe.id}
                onClick={() => { updateSlot(selectedDaySlot.day, selectedDaySlot.slotId, recipe); setSelectedDaySlot(null); }}
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto pb-24 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
        ...
        <div className="flex bg-slate-800 p-1 rounded-xl border border-slate-700">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'rapido' ? 'bg-teal-500 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
          >
            Menú Rápido
          </button>
          <button 
            onClick={() => setActiveTab('experto')}
            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'experto' ? 'bg-teal-500 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
          >
            A Tu Gusto
          </button>
        </div>
      ...
      {activeTab === 'rapido' && (
        <div className="bg-gradient-to-r from-slate-800 to-slate-900 border border-slate-700 rounded-3xl p-8 md:p-12 text-center relative overflow-hidden shadow-2xl">
           <div className="relative z-10 max-w-2xl mx-auto">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">Semana Lista en 1 Segundo</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button 
                ...
                className="bg-teal-500 hover:bg-teal-400 text-slate-900 font-bold py-3 px-6 rounded-xl shadow-lg shadow-teal-500/20 transition-all hover:scale-105 flex items-center justify-center gap-2"
              >
      ...
      {activeTab === 'experto' && (
        <div className="space-y-8 animate-in slide-in-from-bottom-4">
          {/* Info Proteína */}
          <div className="bg-indigo-900/20 border border-indigo-500/30 p-4 rounded-xl flex gap-4 items-center">
            <div className="p-2 bg-indigo-500/20 rounded-lg text-indigo-400"><Dumbbell className="w-5 h-5" /></div>
            <p className="text-indigo-200 text-xs md:text-sm">
              <strong>Meta de Proteína:</strong> Mantente en la <span className="text-emerald-400 font-bold">Zona Verde (+90g)</span> para proteger tu músculo y acelerar el metabolismo.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {days.map((day) => {
              ...
              let proteinColor = "bg-yellow-500";
              if (dailyProtein > 90) proteinColor = "bg-emerald-500";

              return (
                <div key={day} className="bg-slate-800/40 border border-slate-700 rounded-xl overflow-hidden flex flex-col">
                  <div className="p-3 bg-slate-800 border-b border-slate-700 flex justify-between items-center">
                    <h3 className="font-bold text-white text-sm">{day}</h3>
                    <div className="flex flex-col items-end w-1/2">
                        <span className="text-[10px] text-slate-400 mb-1">{dailyProtein}g Prot.</span>
                        <div className="w-full bg-slate-700 h-1.5 rounded-full overflow-hidden">
                           <div className={`h-full ${proteinColor} transition-all duration-500`} style={{ width: `${Math.min((dailyProtein / 110) * 100, 100)}%` }}></div>
                        </div>
                    </div>
                  </div>
                        ...
                        <div key={slot.id} className="relative group">
                          <div className="absolute right-2 top-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                             <button 
                               ...
                               className="p-1 bg-slate-900 rounded-full text-teal-400 hover:text-white border border-slate-600"
                          ...
                          <div 
                            ...
                            className={`p-2 rounded-lg border transition-all cursor-pointer min-h-[60px] flex flex-col justify-center relative
                              ${recipe ? 'bg-slate-700/40 border-slate-600 hover:border-teal-500' : 'bg-slate-800/30 border-dashed border-slate-700'}`}
                          >
                            <div className="flex items-center gap-2 mb-1">
                                <Icon className="w-3 h-3 text-slate-500" />
                                <span className="text-[9px] uppercase font-bold text-slate-500">{slot.label.split(' ')[0]}</span>
                            </div>
                            {recipe ? (
                                <p className="text-xs font-medium text-slate-200 line-clamp-2">{recipe.name}</p>
                            ) : (
                                <div className="text-center text-slate-600 text-xs">+ Agregar</div>
                            )}
                          </div>
                        </div>
                      );
           ...
           <div className="flex justify-center pt-6">
             <button onClick={() => setShowShoppingList(!showShoppingList)} className="bg-white text-slate-900 font-bold py-3 px-8 rounded-full shadow-xl hover:bg-slate-100 transition-all flex items-center gap-2">
                <ShoppingCart className="w-5 h-5 text-teal-600" />
                {showShoppingList ? 'Ocultar Lista' : 'Generar Lista de Compras'}
              </button>
          </div>
        </div>
      )}
