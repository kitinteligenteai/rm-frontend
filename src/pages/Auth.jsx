...
// Versión v2.4 - Fix Syntax Error en Botón Password

import React, { useState, useEffect } from 'react';
import { useUser } from '../context/UserContext.jsx'; 
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { Mail, Lock, User, Loader2, CheckCircle, AlertTriangle, Eye, EyeOff, ArrowLeft } from 'lucide-react';
import { supabase } from '../lib/supabaseClient';

// Componente Input Mejorado con Toggle Password (CORREGIDO)
...
const AuthPage = () => {
  ...
  const [fullName, setFullName] = useState(''); // Nuevo estado para nombre
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      const message = errorMessages[err.message] || err.message || 'Ha ocurrido un error inesperado.';
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-400">
            ...
            {mode === 'signup' && 'Únete al Reinicio Metabólico.'}
        ...
        <motion.form variants={formVariants} initial="hidden" animate="visible" onSubmit={handleAuth} className="space-y-5">
          ...
          {mode === 'signup' && (
            <motion.div variants={formVariants}>
              <AuthInput id="name" type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Nombre Completo" icon={User} disabled={isFormDisabled} />
            </motion.div>
          )}

          <motion.div variants={formVariants}>
            <AuthInput id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Correo electrónico" icon={Mail} disabled={isFormDisabled} />
          </motion.div>
          
          {mode !== 'reset' && (
            <motion.div variants={formVariants}>
              <AuthInput id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Contraseña" icon={Lock} disabled={isFormDisabled} />
            </motion.div>
          )}
