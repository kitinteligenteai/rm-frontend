...
// v6.1 - FIX: Extensión GIF correcta
...
const ChefDanteWidget = () => {
  ...
  // ✅ CORRECCIÓN: Apuntamos al archivo .gif que generaste
        ...
        {isOpen && currentMessage && (
          ...
          >
            ...
            <div className="flex gap-3 mt-3 items-center pt-2 border-t border-slate-100">
                {currentMessage.action && <Link to={currentMessage.action} className="text-xs font-bold text-white bg-indigo-600 px-3 py-1.5 rounded-full hover:bg-indigo-500 transition-colors flex items-center gap-1">Ir ahora <ExternalLink size={10} /></Link>}
                <button onClick={handleNextTip} className="text-xs text-slate-400 hover:text-indigo-500 flex items-center gap-1 ml-auto"><RefreshCw size={12} /> Otro tip</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
